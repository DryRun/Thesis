...
\begin{figure}[htbp]
  \centering
  \hfill
  \subfloat[] {\label{fig:qmu-pdfs-good}
    \resizebox{0.4\textwidth}{!}{\includegraphics{figures/appendix/CLs_example_good}}
  }
  \hfill
  \subfloat[] {\label{fig:qmu-pdfs-bad}
    \resizebox{0.39\textwidth}{!}{\includegraphics{figures/appendix/CLs_example_bad}}
  }
  \hfill
  \caption{Two example of pdfs for $\tilde{q}_{\mu}$ for the $S+B$ and $B$ hypotheses~\cite{atlascls}. The left plot shows pdfs where the two hypotheses are well separated, while the right plot shows a case where the experiment has little sensitivity to the signal.}
  \label{fig:qmu-pdfs}
\end{figure}
